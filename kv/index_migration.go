...
func NewIndexMigration(mapping IndexMapping, store SchemaStore, opts ...IndexMigrationOption) *IndexMigration {
	m := &IndexMigration{
...
func (i *IndexMigration) Up(ctx context.Context) (err error) {
	...
	if err = i.store.CreateBucket(ctx, i.IndexBucket()); err != nil {
		return wrapErr(err)
	}

	_, err = i.Populate(ctx)
	return wrapErr(err)
}

// Down deletes all entries from the index.
func (i *IndexMigration) Down(ctx context.Context) error {
	if err := i.store.DeleteBucket(ctx, i.IndexBucket()); err != nil {
...
func (i *IndexMigration) Populate(ctx context.Context) (n int, err error) {
	...
	diff, err := indexVerify(ctx, i, i.store, i.removeDanglingForeignKeys)
	if err != nil {
	...
	flush := func(batch kvSlice) error {
		...
		if err := i.store.Update(ctx, func(tx Tx) error {
	...
	if i.removeDanglingForeignKeys {
		return n, i.remove(ctx, diff.MissingFromSource)
	}

	return n, nil
}

func (i *IndexMigration) remove(ctx context.Context, mappings map[string]map[string]struct{}) error {
	var (
		batch [][]byte
