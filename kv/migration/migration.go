...
// Store is a superset of kv.Store along with kv store schema change
// functionality like bucket creation and deletion.
type Store interface {
	kv.Store

	// CreateBucket creates a bucket on the underlying store if it does not
	// already exist
	CreateBucket(ctx context.Context, bucket []byte) error
	// DeleteBucket deletes a bucket on the underlying store if it exists already
...
func (a AnonymousMigration) Up(ctx context.Context) error { return a.up(ctx) }

// Down calls the underlying down migration func.
func (a AnonymousMigration) Down(ctx context.Context) error { return a.down(ctx) }
...
func (m *Migrator) Up(ctx context.Context) error {
	...
	for idx, spec := range m.Specs[lastMigration:] {
		...
		if err := m.putMigration(ctx, migration); err != nil {
			return wrapErr(err)
		}

		if err := spec.Up(ctx); err != nil {
			return wrapErr(err)
		}
...
func (m *Migrator) Down(ctx context.Context) (err error) {
	...
	for i := len(migrations) - 1; i >= 0; i-- {
		...
		if err := migration.Spec.Down(ctx); err != nil {
			return wrapErr(err)
		}

		if err := m.deleteMigration(ctx, migration.Migration); err != nil {
			return wrapErr(err)
		}
...
func (m *Migrator) putMigration(ctx context.Context, migration Migration) error {
...
func (m *Migrator) deleteMigration(ctx context.Context, migration Migration) error {
	return m.store.Update(ctx, func(tx kv.Tx) error {
		bkt, err := tx.Bucket(migrationBucket)
		if err != nil {
			return err
